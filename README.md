...
- **Manual canary release workflow** — `.github/workflows/canary.yml` publishes `{base}-canary.{N}` builds of `gsd-qwen` and `@gsd-build/sdk` to the `@canary` dist-tag from `dev` on demand via `workflow_dispatch`.
...
> [!NOTE]
> Claude Code 2.1.88+, Qwen Code, and Codex install as skills (`.qwen/skills/`, `./.codex/skills/`, or the matching global `~/.qwen/skills/` / `~/.codex/skills/` roots). Older Claude Code versions use `commands/gsd/`. `~/.qwen/get-shit-done/skills/` is import-only for legacy migration. The installer handles all formats automatically.
...
```bash
# Claude Code
npx gsd-qwen --claude --global   # Install to ~/.qwen/
npx gsd-qwen --claude --local    # Install to ./.qwen/

# OpenCode
npx gsd-qwen --opencode --global # Install to ~/.config/opencode/

# Gemini CLI
npx gsd-qwen --gemini --global   # Install to ~/.gemini/

# Kilo
npx gsd-qwen --kilo --global     # Install to ~/.config/kilo/
npx gsd-qwen --kilo --local      # Install to ./.kilo/

# Codex
npx gsd-qwen --codex --global    # Install to ~/.codex/
npx gsd-qwen --codex --local     # Install to ./.codex/

# Copilot
npx gsd-qwen --copilot --global  # Install to ~/.github/
npx gsd-qwen --copilot --local   # Install to ./.github/

# Cursor CLI
npx gsd-qwen --cursor --global      # Install to ~/.cursor/
npx gsd-qwen --cursor --local       # Install to ./.cursor/

# Windsurf
npx gsd-qwen --windsurf --global    # Install to ~/.codeium/windsurf/
npx gsd-qwen --windsurf --local     # Install to ./.windsurf/

# Antigravity
npx gsd-qwen --antigravity --global # Install to ~/.gemini/antigravity/
npx gsd-qwen --antigravity --local  # Install to ./.agent/

# Augment
npx gsd-qwen --augment --global     # Install to ~/.augment/
npx gsd-qwen --augment --local      # Install to ./.augment/

# Trae
npx gsd-qwen --trae --global        # Install to ~/.trae/
npx gsd-qwen --trae --local         # Install to ./.trae/

# Qwen Code
npx gsd-qwen --qwen --global        # Install to ~/.qwen/
npx gsd-qwen --qwen --local         # Install to ./.qwen/

# Hermes Agent
npx gsd-qwen --hermes --global      # Install to ~/.hermes/ (honors $HERMES_HOME)
npx gsd-qwen --hermes --local       # Install to ./.hermes/

# CodeBuddy
npx gsd-qwen --codebuddy --global   # Install to ~/.codebuddy/
npx gsd-qwen --codebuddy --local    # Install to ./.codebuddy/

# Cline
npx gsd-qwen --cline --global       # Install to ~/.cline/
npx gsd-qwen --cline --local        # Install to ./.clinerules

# All runtimes
npx gsd-qwen --all --global      # Install to all directories
```
...
```bash
npx gsd-qwen --claude --global --minimal
# or any other runtime — works the same
npx gsd-qwen --opencode --global --minimal
```
...
> [!IMPORTANT]
> **The savings disappear the moment you re-install without `--minimal`.** Running `npx gsd-qwen@latest` (or `gsd update` from inside a session) without the flag puts the full 86-skill / 33-agent surface back on disk, and every subsequent session pays the full ~12k-token floor again. If you want to stay minimal, **always pass `--minimal` when updating**:
>
> ```bash
> npx gsd-qwen@latest --claude --global --minimal
> ```
>
...
>
> Tip: `cat ~/.qwen/get-shit-done/.gsd-manifest.json | jq .mode` (or `gsd-file-manifest.json` depending on layout) confirms which mode you're in.
...
Installs to `./.qwen/` for testing modifications before contributing.
...
1. Open Claude Code settings (`.qwen/settings.json` or global)
...
- Verify files exist in `~/.qwen/skills/gsd-*/SKILL.md` or `~/.codex/skills/gsd-*/SKILL.md` for managed global installs
- For local installs, verify `.qwen/skills/gsd-*/SKILL.md` or `./.codex/skills/gsd-*/SKILL.md`
- Legacy Claude Code installs still use `~/.qwen/commands/gsd/`

**Commands not working as expected?**
- Run `/gsd-help` to verify installation
- Re-run `npx gsd-qwen` to reinstall
...
If file reads fail with tilde paths (`~/.qwen/...`), set `CLAUDE_CONFIG_DIR` before installing:
```bash
...
npx gsd-qwen --gemini --global --uninstall
npx gsd-qwen --kilo --global --uninstall
npx gsd-qwen --codex --global --uninstall
npx gsd-qwen --copilot --global --uninstall
npx gsd-qwen --cursor --global --uninstall
npx gsd-qwen --windsurf --global --uninstall
npx gsd-qwen --antigravity --global --uninstall
npx gsd-qwen --augment --global --uninstall
npx gsd-qwen --trae --global --uninstall
npx gsd-qwen --qwen --global --uninstall
npx gsd-qwen --hermes --global --uninstall
npx gsd-qwen --codebuddy --global --uninstall
npx gsd-qwen --cline --global --uninstall

# Local installs (current project)
npx gsd-qwen --claude --local --uninstall
npx gsd-qwen --opencode --local --uninstall
npx gsd-qwen --gemini --local --uninstall
npx gsd-qwen --kilo --local --uninstall
npx gsd-qwen --codex --local --uninstall
npx gsd-qwen --copilot --local --uninstall
npx gsd-qwen --cursor --local --uninstall
npx gsd-qwen --windsurf --local --uninstall
npx gsd-qwen --antigravity --local --uninstall
npx gsd-qwen --augment --local --uninstall
npx gsd-qwen --trae --local --uninstall
npx gsd-qwen --qwen --local --uninstall
npx gsd-qwen --hermes --local --uninstall
npx gsd-qwen --codebuddy --local --uninstall
npx gsd-qwen --cline --local --uninstall
```
...
OpenCode, Gemini CLI, Kilo, and Codex are now natively supported via `npx gsd-qwen`.
