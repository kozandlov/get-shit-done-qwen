...
- **手动 canary 发布工作流** — `.github/workflows/canary.yml` 通过 `workflow_dispatch` 从 `dev` 分支按需将 `{base}-canary.{N}` 构建（`gsd-qwen` 与 `@gsd-build/sdk`）发布到 `@canary` dist-tag。
...
# Kilo
npx gsd-qwen --kilo --global     # 安装到 ~/.config/kilo/
npx gsd-qwen --kilo --local      # 安装到 ./.kilo/
...
# Copilot
npx gsd-qwen --copilot --global  # 安装到 ~/.github/
npx gsd-qwen --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx gsd-qwen --cursor --global   # 安装到 ~/.cursor/
npx gsd-qwen --cursor --local    # 安装到 ./.cursor/

# Antigravity
npx gsd-qwen --antigravity --global # 安装到 ~/.gemini/antigravity/
npx gsd-qwen --antigravity --local  # 安装到 ./.agent/

# Augment
npx gsd-qwen --augment --global     # 安装到 ~/.augment/
npx gsd-qwen --augment --local      # 安装到 ./.augment/

# Trae
npx gsd-qwen --trae --global     # 安装到 ~/.trae/
npx gsd-qwen --trae --local      # 安装到 ./.trae/

# CodeBuddy
npx gsd-qwen --codebuddy --global # 安装到 ~/.codebuddy/
npx gsd-qwen --codebuddy --local  # 安装到 ./.codebuddy/

# Cline
npx gsd-qwen --cline --global       # 安装到 ~/.cline/
npx gsd-qwen --cline --local        # 安装到 ./.clinerules
...
如果你不想使用这个 flag，可以在项目的 `.qwen/settings.json` 中加入：
...
1. 打开 Claude Code 设置（项目级 `.qwen/settings.json` 或全局设置）
...
如果使用波浪线路径（`~/.qwen/...`）时读取失败，请在安装前设置 `CLAUDE_CONFIG_DIR`：
```bash
...
OpenCode、Gemini CLI、Kilo 和 Codex 现在都已经通过 `npx gsd-qwen` 获得原生支持。
