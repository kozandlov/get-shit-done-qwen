...
**一个轻量但强大的元提示、上下文工程与规格驱动开发系统，适用于 Qwen Code CLI。**

**它解决的是 context rot：随着上下文窗口被填满，输出质量逐步劣化的问题。**
...
*"这是我给 Qwen Code CLI 加过最强的增强。没有过度设计，是真的把事做完。"*
...
这就是它。没有企业角色扮演式的废话，只有一套非常有效、能让你持续用 Qwen Code CLI 构建酷东西的系统。
...
GSD 解决的就是这个问题。它是让 Qwen Code CLI 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Qwen Code CLI 去干活。
...
GSD 的设计目标是无摩擦自动化。运行 Qwen Code CLI 时建议使用对应的无确认/自动批准模式。

> [!TIP]
> 如果你的 Qwen CLI 提供无确认/自动批准模式，建议开启。GSD 的预期用法是不停地弹确认会显著拖慢流程。
...
Qwen Code CLI 非常强大，前提是你把它需要的上下文给对。大多数人做不到。
...
GSD 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Qwen Code CLI 的 deny list**：

1. 打开 Qwen Code CLI 设置（项目级 `.qwen/settings.json` 或全局设置）
...
如果使用波浪线路径（`~/.qwen/...`）时读取失败，请在安装前设置 `QWEN_CONFIG_DIR`（`CLAUDE_CONFIG_DIR` 仍兼容）：
```bash
