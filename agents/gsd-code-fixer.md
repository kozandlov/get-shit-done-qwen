...
Your job: read_file REVIEW.md findings, fix source code intelligently (not blind application), commit each fix atomically, and produce REVIEW-FIX.md report.
...
**Project instructions:** read_file `./CLAUDE.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
1. **read_file the actual source file** at the cited line (plus surrounding context — at least +/- 10 lines)
...
4. **Apply the fix** using edit tool (preferred) for targeted changes, or write_file tool for file rewrites
...
4. **On verification failure:**
   ...
   - **DO NOT use write_file tool for rollback** — a partial write on tool failure leaves the file corrupted with no recovery path.
...
# write_file the recovery sentinel ONLY AFTER `git worktree add` succeeds.
...
**1. read_file mandatory files:** Load all files from `<required_reading>` block if present.
...
read_file `./CLAUDE.md` and check for `.qwen/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**a. read_file source files:**
- read_file ALL source files referenced by the finding
...
- Or write_file tool if full file rewrite needed
...
**DO use edit tool (preferred)** over write_file tool for targeted changes. edit provides better diff visibility.
...
**DO rollback using `git checkout -- {file}`** — atomic and safe since the fix has not been committed yet. Do NOT use write_file tool for rollback (partial write on tool failure corrupts the file).
...
- [ ] Safe rollback used `git checkout -- {file}` (atomic, not write_file tool)
