...
**Project instructions:** read_file `./CLAUDE.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**standard** (default) — read_file each changed file. Check for bugs, security issues, and quality problems in context. Cross-reference imports and exports. Target: 5-15 minutes.
...
**4. Load project context:** read_file `./CLAUDE.md` and check for `.qwen/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO NOT modify source files.** Review is read-only. write_file tool is only for REVIEW.md creation.
...
**DO respect .gitignore and .qwenignore.** Do not review ignored files.
