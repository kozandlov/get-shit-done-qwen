...
read_file the focus area from your prompt. It will be one of: `tech`, `arch`, `quality`, `concerns`.
...
When present, restrict your exploration (glob/grep_search/run_shell_command globs) to files under the listed repo-relative path prefixes. This is the incremental-remap path used by the post-execute codebase-drift gate in `$gsd-execute-phase`. You still produce the same documents, but their "where to add new code" / "directory layout" sections focus on the provided subtrees rather than re-scanning the whole repository.
...
read_file key files identified during exploration. Use glob and grep_search liberally.
</step>

<step name="write_documents">
write_file document(s) to `.planning/codebase/` using the templates below.
...
**BE THOROUGH.** Explore deeply. read_file actual files. Don't guess. **But respect <forbidden_files>.**
