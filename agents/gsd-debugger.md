...
<required_reading>
@~/.qwen/get-shit-done/references/common-bug-patterns.md
</required_reading>
...
@~/.qwen/get-shit-done/references/debugger-philosophy.md
...
**write_file this block to Current Focus BEFORE starting fix_and_verify:**
...
**`next_action` must be concrete and actionable.** Bad examples: "continue investigating", "look at the code". Good examples: "Add logging at line 47 of auth.js to observe token value before jwt.verify()", "Run test suite with NODE_ENV=production to check env-specific behavior", "read_file full implementation of getUserById in db/users.cjs".
...
@~/.qwen/get-shit-done/references/thinking-models-debug.md
...
- read_file @~/.qwen/get-shit-done/references/common-bug-patterns.md
...
- write_file the `reasoning_checkpoint` block to Current Focus (see Structured Reasoning Checkpoint in investigation_techniques)
...
- Before entering fix_and_verify, enter tdd_debug_mode:
  1. write_file a minimal failing test that directly exercises the bug
