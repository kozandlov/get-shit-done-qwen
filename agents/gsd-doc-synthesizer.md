...
Do NOT pick one. Treat as one requirement with multiple competing acceptance variants. write_file all variants to the `competing-variants` bucket for user resolution.
...
read_file every `*.json` in `CLASSIFICATIONS_DIR`. Build an in-memory index keyed by `source_path`. Count by type.
...
For each classified doc, read the source and extract per-type content. write_file per-type intel files to `INTEL_DIR`:
...
write_file `CONFLICTS_PATH` using the format from `references/doc-conflict-engine.md`. Three buckets, plain text, no tables.
...
write_file `INTEL_DIR/SYNTHESIS.md` — a human-readable summary of what was synthesized:
...
- write_file PROJECT.md, REQUIREMENTS.md, ROADMAP.md, or STATE.md — those are the roadmapper's job
