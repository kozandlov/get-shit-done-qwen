...
you MUST read_file every listed file BEFORE any other action.
...
write_file machine-parseable, evidence-based intelligence. Every claim references actual file paths. Prefer structured JSON over prose.

- **Always include file paths.** Every claim must reference the actual code location.
- **write_file current state only.** No temporal language ("recently added", "will be changed").
- **Evidence-based.** read_file the actual files. Do not guess from file names or directory structures.
- **Cross-platform.** Use glob, read_file, and grep_search tools -- not run_shell_command `ls`, `find`, or `cat`. run_shell_command file commands fail on Windows. Only use run_shell_command for `node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel` CLI calls.
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel extract-exports <file>` to get accurate exports.
...
read_file package.json, configs, and build files. write_file `stack.json`. Then patch its timestamp:
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/stack.json --cwd <project_root>
```

### Step 3: File Graph

glob source files (`**/*.ts`, `**/*.js`, `**/*.py`, etc., excluding node_modules/dist/build).
read_file key files (entry points, configs, core modules) for imports/exports.
write_file `files.json`. Then patch its timestamp:
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/files.json --cwd <project_root>
```
...
grep_search for route definitions, endpoint declarations, CLI command registrations.
Patterns to search: `app.get(`, `router.post(`, `@GetMapping`, `def route`, express route patterns.
write_file `apis.json`. If no API endpoints found, write an empty entries object. Then patch its timestamp:
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/apis.json --cwd <project_root>
```

### Step 5: Dependencies

read_file package.json (dependencies, devDependencies), requirements.txt, go.mod, Cargo.toml.
Cross-reference with actual imports to populate `used_by`.
write_file `deps.json`. Then patch its timestamp:
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel validate --cwd <project_root>`
...
Run: `node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel snapshot --cwd <project_root>`
...
4. read_file existing intel files first, merge updates, write back
...
- [ ] All entries reference actual file paths verified by glob/read_file
...
| 50-70% | DEGRADING | write_file incrementally, skip non-essential |
...
2. DO NOT use run_shell_command for file listing -- use glob tool
