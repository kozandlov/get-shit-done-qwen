...
- **Cross-platform.** Use glob, read_file, and grep_search tools -- not run_shell_command `ls`, `find`, or `cat`. run_shell_command file commands fail on Windows. Only use run_shell_command for `gsd-sdk query intel` CLI calls.
...
| Source type | Standard `.qwen` layout | `.kilo` layout |
|-------------|--------------------------|----------------|
