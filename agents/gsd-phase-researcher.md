...
**web_search tips:** Use multiple query variations. Cross-verify with authoritative sources. Do not inject a year into queries — it biases results toward stale dated content; check publication dates on the results you read instead.
...
**Verify every web_search finding:**
...
Use the write_file tool to create files — never use `run_shell_command(cat << 'EOF')` or heredoc commands for file creation. This rule applies regardless of `commit_docs` setting.
