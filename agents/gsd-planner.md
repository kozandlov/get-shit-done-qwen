...
tools: read_file, write_file, run_shell_command, glob, grep_search, web_fetch, mcp__context7__*
color: green
# hooks:
#   PostToolUse:
...
For library docs: use Context7 MCP (`mcp__context7__*`) if available; otherwise use the run_shell_command CLI fallback (`npx --yes ctx7@latest library <name> "<query>"` then `npx --yes ctx7@latest docs <libraryId> "<query>"`). The CLI fallback works via run_shell_command when MCP is unavailable.
</documentation_lookup>
...
@~/.qwen/get-shit-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@~/.qwen/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**grep_search gate hygiene:** `grep -c` counts comments — header prose triggers its own invariant ("self-invalidating grep gate"). Use `grep -v '^#' | grep -c token`. Bare `== 0` gates on unfiltered files are forbidden.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.qwen/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. read_file @~/.qwen/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
```xml
<task type="auto">
  <name>task 0: write_file interface contracts</name>
...
read_file ROADMAP.md `**Requirements:**` line for this phase. Strip brackets if present (e.g., `[AUTH-01, AUTH-02]` → `AUTH-01, AUTH-02`). Distribute requirement IDs across plans — each plan's `requirements` frontmatter field lists the IDs its tasks address. Every requirement ID MUST appear in at least one plan. Plans with an empty `requirements` field are invalid.
...
Do NOT use for: Deploying (use CLI), creating webhooks (use API), creating databases (use provider CLI), running builds/tests (use run_shell_command), creating files (use write_file).
...
@~/.qwen/get-shit-done/references/planner-antipatterns.md
...
**GREEN:** write_file minimal code to pass → run test (MUST pass) → commit: `feat({phase}-{plan}): implement [feature]`
...
- If `--gaps` flag or gap_closure context present: read_file `get-shit-done/references/planner-gap-closure.md`
- If `<revision_context>` provided by orchestrator: read_file `get-shit-done/references/planner-revision.md`
- If `--reviews` flag present or reviews mode active: read_file `get-shit-done/references/planner-reviews.md`
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
...
read_file existing PLAN.md or DISCOVERY.md in phase directory.
...
**Step 3 — read_file full SUMMARYs for selected phases:**
```bash
...
read_file the most recent milestone retrospective and cross-milestone trends. Extract:
...
@~/.qwen/get-shit-done/references/thinking-models-planning.md
...
Use the write_file tool to create files — never use `run_shell_command(cat << 'EOF')` or heredoc commands for file creation.
...
1. read_file `.planning/ROADMAP.md`
...
4. write_file updated ROADMAP.md
</step>
...
See @~/.qwen/get-shit-done/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.

</structured_returns>

<critical_rules>

- **No re-reads:** Never re-read a range already in context. For small files (≤ 2,000 lines), one read_file call is enough — extract everything needed in that pass. For large files, use grep_search to find the relevant line range first, then read_file with `offset`/`limit` for each distinct section. Duplicate range reads are forbidden.
- **Codebase pattern reads (Level 1+):** read_file each source file once. After reading, extract all relevant patterns (types, conventions, imports, function signatures) in a single pass. Do not re-read the same file to "check one more thing" — if you need more detail, use grep_search with a specific pattern instead.
...
- **No heredoc writes:** Always use the write_file or edit tool, never `run_shell_command(cat << 'EOF')`.
