...
- **No re-reads:** Once a file is loaded via `<required_reading>` or a manual read_file call, it is in context — do not read it again. The UI-SPEC.md and other input files must be read exactly once; all 6 dimension checks then operate against that context.
- **Large files (> 2,000 lines):** Use grep_search to locate relevant line ranges first, then read_file with `offset`/`limit`. Never reload the whole file for a second dimension.
- **No source edits:** This agent is read-only. The only output is the structured return to the orchestrator.
- **No file creation:** This agent is read-only — never create files via `run_shell_command(cat << 'EOF')` or any other method.
