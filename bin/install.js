...
Then re-run: npx gsd-qwen@latest
`);
    process.exit(1);
  }
}
...
function getGlobalDir(runtime, explicitDir = null) {
  ...
  // Claude Code: --config-dir > CLAUDE_CONFIG_DIR > ~/.qwen
  if (explicitDir) {
    return expandTilde(explicitDir);
  }
...
function buildHookCommand(configDir, hookName, opts) {
  ...
  if (opts.portableHooks) {
    ...
    // ~/.qwen is bind-mounted into a container at a different absolute path.
...
/**
 * read_file and parse settings.json, returning empty object if it doesn't exist.
...
/**
 * write_file settings.json with proper formatting
 */
...
/**
 * read_file model_overrides from ~/.gsd/defaults.json at install time.
...
/**
 * #2517 — read_file a single GSD config file (defaults.json or per-project
...
const claudeToOpencodeTools = {
  ...
  web_fetch: 'webfetch',
  web_search: 'websearch',  // Plugin/MCP - keep for compatibility
};
 ...
 * - Applies special mappings (ask_user_question -> question, etc.)
 ...
 * - Applies Claude→Gemini mapping (read_file→read_file, run_shell_command→run_shell_command, etc.)
 ...
 *   Global: ~/.qwen/ → ~/.copilot/, ./.qwen/ → ./.github/
 *   Local:  ~/.qwen/ → ./.github/, ./.qwen/ → ./.github/
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  // Handle both `~/.qwen/foo` (trailing slash) and bare `~/.qwen` forms in
  // one pass via a capture group, matching the approach used by Antigravity,
  // OpenCode, Kilo, and Codex converters (issue #2545).
  if (isGlobal) {
    c = c.replace(/\$HOME\/\.qwen(\/|\b)/g, '$HOME/.copilot$1');
    c = c.replace(/~\/\.qwen(\/|\b)/g, '~/.copilot$1');
  } else {
    c = c.replace(/\$HOME\/\.qwen\//g, '.github/');
    c = c.replace(/~\/\.qwen\//g, '.github/');
    c = c.replace(/\$HOME\/\.qwen\b/g, '.github');
    c = c.replace(/~\/\.qwen\b/g, '.github');
  }
  c = c.replace(/\.\/\.qwen\//g, './.github/');
  c = c.replace(/\.qwen\//g, '.github/');
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.qwen/ → ~/.gemini/antigravity/, ./.qwen/ → ./.agent/
 *   Local:  ~/.qwen/ → .agent/, ./.qwen/ → ./.agent/
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  let c = content;
  if (isGlobal) {
    c = c.replace(/\$HOME\/\.qwen\//g, '$HOME/.gemini/antigravity/');
    c = c.replace(/~\/\.qwen\//g, '~/.gemini/antigravity/');
    // Bare form (no trailing slash) — must come after slash form to avoid double-replace
    c = c.replace(/\$HOME\/\.qwen\b/g, '$HOME/.gemini/antigravity');
    c = c.replace(/~\/\.qwen\b/g, '~/.gemini/antigravity');
  } else {
    c = c.replace(/\$HOME\/\.qwen\//g, '.agent/');
    c = c.replace(/~\/\.qwen\//g, '.agent/');
    // Bare form (no trailing slash) — must come after slash form to avoid double-replace
    c = c.replace(/\$HOME\/\.qwen\b/g, '.agent');
    c = c.replace(/~\/\.qwen\b/g, '.agent');
  }
  c = c.replace(/\.\/\.qwen\//g, './.agent/');
  c = c.replace(/\.qwen\//g, '.agent/');
...
const claudeToCursorTools = {
  ...
  edit: 'StrReplace',
  ask_user_question: null, // No direct equivalent — use conversational prompting
...
function convertCursorToolName(claudeTool) {
  ...
  // Most tools share the same name (read_file, write_file, glob, grep_search, task, web_search, web_fetch, todo_write)
  return claudeTool;
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.cursor/skills/');
...
- \`read_file\`, \`write_file\`, \`glob\`, \`grep_search\`, \`task\`, \`web_search\`, \`web_fetch\`, \`todo_write\` as needed
...
function convertClaudeToWindsurfMarkdown(content) {
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.windsurf/skills/');
  // Remove Claude Code-specific bug workarounds before brand replacement
  converted = converted.replace(/\*\*Known Claude Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
...
const claudeToAugmentTools = {
  run_shell_command: 'launch-process',
...
function convertClaudeToAugmentMarkdown(content) {
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.augment/skills/');
  // Remove Claude Code-specific bug workarounds before brand replacement
  converted = converted.replace(/\*\*Known Claude Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
...
function copyCommandsAsAugmentSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  function recurse(currentSrcDir, currentPrefix) {
    ...
    for (const entry of entries) {
      ...
      const globalClaudeRegex = /~\/\.qwen\//g;
      const globalClaudeHomeRegex = /\$HOME\/\.qwen\//g;
      const localClaudeRegex = /\.\/\.qwen\//g;
...
function convertClaudeToTraeMarkdown(content) {
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.trae/skills/');
  converted = converted.replace(/\.\/\.qwen\//g, './.trae/');
  converted = converted.replace(/\.qwen\//g, '.trae/');
  converted = converted.replace(/\*\*Known Claude Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
...
function convertClaudeToCodebuddyMarkdown(content) {
  ...
  // CodeBuddy uses the same tool names as Claude Code (run_shell_command, edit, read_file, write_file, etc.)
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.codebuddy/skills/');
  converted = converted.replace(/\.\/\.qwen\//g, './.codebuddy/');
  converted = converted.replace(/\.qwen\//g, '.codebuddy/');
  converted = converted.replace(/\*\*Known Claude Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
...
function convertClaudeToCliineMarkdown(content) {
  ...
  converted = converted.replace(/\.qwen\/skills\//g, '.cline/skills/');
  converted = converted.replace(/\.\/\.qwen\//g, './.cline/');
  converted = converted.replace(/\.qwen\//g, '.cline/');
  converted = converted.replace(/\*\*Known Claude Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
...
function convertClaudeToCodexMarkdown(content) {
  ...
  // Path replacement: .qwen → .codex (#1430)
  converted = converted.replace(/\$HOME\/\.qwen\//g, '$HOME/.codex/');
  converted = converted.replace(/~\/\.qwen\//g, '~/.codex/');
  converted = converted.replace(/\.\/\.qwen\//g, './.codex/');
  // Bare/project-relative .qwen/... references (#2639). Covers strings like
  // "check `.qwen/skills/`" where there is no ~/, $HOME/, or ./ anchor.
  ...
  converted = converted.replace(/(?<![A-Za-z0-9_\-./~$])\.qwen\//g, '.codex/');
  // `.qwenignore` → `.codexignore` (#2639). Codex honors its own ignore
  // file; leaving the Claude-specific name is misleading in agent prompts.
  converted = converted.replace(/\.qwenignore\b/g, '.codexignore');
  // Runtime-neutral agent name replacement (#766)
...
## B. ask_user_question → request_user_input Mapping
GSD workflows use \`ask_user_question\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
Batched calls:
- \`ask_user_question([q1, q2])\` → single \`request_user_input\` with multiple entries in \`questions[]\`
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  for (const file of agentEntries) {
    ...
    // Replace full .qwen/get-shit-done prefix so path resolves to the Codex
    // GSD install before generic .qwen → .codex conversion rewrites it.
    content = content.replace(/~\/\.qwen\/get-shit-done\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.qwen\/get-shit-done\//g, codexGsdPath);
    ...
    // $ARGUMENTS → {{GSD_ARGS}}, /clear removal, anchored and bare .qwen/
    // paths, .qwenignore → .codexignore, and standalone "Claude" /
 ...
 * - tool names: must use Gemini built-in names (read_file, not read_file)
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace ~/.qwen and $HOME/.qwen with OpenCode's config location
  convertedContent = convertedContent.replace(/~\/\.qwen\b/g, '~/.config/opencode');
  convertedContent = convertedContent.replace(/\$HOME\/\.qwen\b/g, '$HOME/.config/opencode');
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  convertedContent = convertedContent.replace(/\$gsd-/g, '/gsd-');
  // Replace ~/.qwen and $HOME/.qwen with Kilo's config location
  convertedContent = convertedContent.replace(/~\/\.qwen\b/g, '~/.config/kilo');
  convertedContent = convertedContent.replace(/\$HOME\/\.qwen\b/g, '$HOME/.config/kilo');
  convertedContent = convertedContent.replace(/\.\/\.qwen\//g, './.kilo/');
  // Normalize both Claude skill directory variants to Kilo's canonical skills dir.
  convertedContent = replaceRelativePathReference(convertedContent, '.qwen/skills/', '.kilo/skills/');
  convertedContent = replaceRelativePathReference(convertedContent, '.agents/skills/', '.kilo/skills/');
  convertedContent = replaceRelativePathReference(convertedContent, '.qwen/agents/', '.kilo/agents/');
...
function copyCommandsAsTraeSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  function recurse(currentSrcDir, currentPrefix) {
    ...
    for (const entry of entries) {
      ...
      const globalClaudeHomeRegex = /\$HOME\/\.qwen\//g;
      const localClaudeRegex = /\.\/\.qwen\//g;
      const bareGlobalClaudeRegex = /~\/\.qwen\b/g;
      const bareGlobalClaudeHomeRegex = /\$HOME\/\.qwen\b/g;
      const bareLocalClaudeRegex = /\.\/\.qwen\b/g;
  ...
  function recurse(currentSrcDir, currentPrefix) {
    ...
    for (const entry of entries) {
      ...
      if (runtime === 'qwen') {
        ...
        content = content.replace(/\.qwen\//g, '.qwen/');
      }
      ...
      if (runtime === 'hermes') {
        ...
        content = content.replace(/\.qwen\//g, '.hermes/');
      }
      content = processAttribution(content, getCommitAttribution(runtime));
...
/**
 * write_file the Hermes "gsd" category DESCRIPTION.md.
 ...
 *
 * @param {string} targetDir - Resolved runtime config directory (e.g. ~/.qwen)
 * @param {Map<string, string>} saved - Map returned by preserveUserArtifacts
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Replace ~/.qwen/ and $HOME/.qwen/ and ./.qwen/ with runtime-appropriate paths
      ...
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.cursor/skills/');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace($gsd-/gi, 'gsd-');
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.windsurf/skills/');
      ...
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.trae/skills/');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.cline/skills/');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.qwen/skills/');
      jsContent = jsContent.replace(/\.qwen\//g, '.qwen/');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\.qwen\/skills\//g, '.hermes/skills/');
      jsContent = jsContent.replace(/\.qwen\//g, '.hermes/');
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // read_file existing config or create empty object
  let config = {};
  if (fs.existsSync(configPath)) {
    try {
      const content = fs.readFileSync(configPath, 'utf8');
...
/**
 * write_file file manifest after installation for future modification detection
 */
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Replaces $HOME/.qwen/ or ~/.qwen/ so the result is <pathPrefix>get-shit-done/bin/...
  ...
  if (isOpencode || isKilo) {
    ...
    // commands from .qwen/commands/gsd/, not .qwen/skills/
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const gsdSrc = stageSkillsForMode(path.join(src, 'commands', 'gsd'), installMode);
    const gsdDest = path.join(commandsDir, 'gsd');
  ...
  if (isMinimalMode(installMode)) {
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        // Replace ~/.qwen/ and $HOME/.qwen/ as they are the source of truth in the repo
        const dirRegex = /~\/\.qwen\//g;
        const homeDirRegex = /\$HOME\/\.qwen\//g;
        const bareDirRegex = /~\/\.qwen\b/g;
        const bareHomeDirRegex = /\$HOME\/\.qwen\b/g;
        const normalizedPathPrefix = pathPrefix.replace(/\/$/, '');
        if (!isCopilot && !isAntigravity) {
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline) {
    // write_file package.json to force CommonJS mode for GSD scripts
    ...
    // Template paths for the target runtime (replaces '.qwen' with correct config dir)
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // Template .js files to replace '.qwen' with runtime-specific config dir
          // and stamp the current GSD version into the hook version header
          if (entry.endsWith('.js')) {
            let content = fs.readFileSync(srcFile, 'utf8');
            content = content.replace(/'\.qwen'/g, configDirReplacement);
            content = content.replace(/\/\.qwen\//g, `/${getDirName(runtime)}/`);
            content = content.replace(/\.qwen\//g, `${getDirName(runtime)}/`);
            if (isQwen) {
              content = content.replace(/CLAUDE\.md/g, 'QWEN.md');
  ...
  // write_file file manifest for future modification detection
  ...
  // Verify no leaked .qwen paths in non-Claude runtimes
  if (runtime !== 'claude') {
    const leakedPaths = [];
    function scanForLeakedPaths(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const matches = content.match(/(?:~|\$HOME)\/\.qwen\b/g);
          if (matches) {
    ...
    if (leakedPaths.length > 0) {
      ...
      console.warn(`\n  ${yellow}⚠${reset}  Found ${totalLeaks} unreplaced .qwen path reference(s) in ${leakedPaths.length} file(s):`);
  ...
  if (!isOpencode && !isKilo) {
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      settings.hooks[postToolEvent].push({
        matcher: 'run_shell_command|edit|write_file|MultiEdit|Agent|task',
        hooks: [
          {
            type: 'command',
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('gsd-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
            entry.matcher = 'run_shell_command|edit|write_file|MultiEdit|Agent|task';
            migrated = true;
          }
          for (const h of entry.hooks) {
    ...
    // Scans content returned by the read_file tool for injection patterns, including
    ...
    // causes a hook error on every run_shell_command tool invocation.
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  // write_file settings when runtime supports settings.json.
 ...
 * gsd-qwen npm tarball. The parent package declares a bin entry
 ...
 *       User ran `npx gsd-qwen@latest`. sdk/ lives under
 *       `<npm-cache>/_npx/<hash>/node_modules/gsd-qwen/sdk` which
 ...
 *       User ran a global install (`npm i -g gsd-qwen`). sdk/dist
 ...
 *     fix_command: 'npm install -g gsd-qwen@latest' | 'cd sdk && npm install && npm run build',
...
function buildSdkFailFastReport(sdkDir, sdkCliPath) {
  ...
  if (ctx.mode === 'tarball') {
    ...
    fix_command = 'npm install -g gsd-qwen@latest';
  } else {
    context = 'dev-clone';
...
function renderSdkFailFastReport(ir) {
  ...
  if (ir.context === 'npx-cache') {
    ...
    console.error(`    ${cyan}npx --yes gsd-qwen@latest${reset}`);
...
function installSdkIfNeeded(opts) {
  ...
  // PATH. `npx gsd-qwen` only links the package's primary bin; the
  ...
  // populated but not on every shell's PATH — Git run_shell_command vs PowerShell vs
 ...
 * (cross-shell PATH probing requires a different strategy — Git run_shell_command
 ...
 * (PowerShell), and a run_shell_command wrapper named `gsd-sdk` (for Cygwin/MSYS/Git-run_shell_command).
...
function formatSdkPathDiagnostic({ shimDir, platform, runDir }) {
  ...
  if (shimDir) {
    ...
    if (isWin32) {
      ...
      actionLines.push(`Git run_shell_command  : echo 'export PATH="${bashShimDir}:$PATH"' >> ~/.bashrc`);
    } else {
    ...
    actionLines.push('npm install -g gsd-qwen');
  }

  const npxNoteLines = isNpx
    ? [
        "Note: you're running via npx. For a persistent shim,",
        'install globally instead: npm install -g gsd-qwen',
      ]
    : [];
...
function trySelfLinkGsdSdkWindows(shimSrc) {
  ...
  try {
    ...
    // WSL-mounted filesystems where run_shell_command users live.
