...
function convertCopilotToolName(claudeTool) {
  ...
  // Support qwen/runtime tool names that already exist in this forked tree.
  const runtimeToolAliases = {
    read_file: 'read',
    write_file: 'edit',
    edit: 'edit',
    replace: 'edit',
  ...
  if (runtimeToolAliases[claudeTool]) {
    return runtimeToolAliases[claudeTool];
  }
  // Default: lowercase
  return claudeTool.toLowerCase();
}
...
/**
 * Convert a runtime-specific or Claude tool name back to the canonical
 * Claude-style label used in Copilot skill frontmatter.
 */
function convertCopilotSkillToolName(toolName) {
  const skillToolAliases = {
    Read: 'Read',
    Write: 'Write',
    Edit: 'Edit',
    Bash: 'Bash',
  ...
  if (skillToolAliases[toolName]) {
    return skillToolAliases[toolName];
  }
...
function convertClaudeCommandToCopilotSkill(content, skillName, isGlobal = false) {
  ...
  if (!frontmatter) return convertedBody;
  ...
  if (toolsMatch) {
    ...
    if (tools) {
      toolsLine = tools
        .map(t => convertCopilotSkillToolName(t.replace(/^\s+-\s+/, '').trim()))
        .join(', ');
    }
  }
