...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  if (isGlobal) {
    c = c.replace(/\$HOME\/\.qwen\//g, '$HOME/.copilot/');
    c = c.replace(/~\/\.qwen\//g, '~/.copilot/');
  } else {
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  for (const file of agentEntries) {
    ...
    // Replace full .qwen/get-shit-done prefix so path resolves to codex GSD install
    content = content.replace(/~\/\.qwen\/get-shit-done\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.qwen\/get-shit-done\//g, codexGsdPath);
    // Replace remaining .qwen paths with .codex equivalents (#2320).
    // Capture group handles both trailing-slash form (~/.qwen/) and
    // bare end-of-string form (~/.qwen) in a single pass.
    content = content.replace(/\$HOME\/\.qwen(\/|$)/g, '$HOME/.codex$1');
    content = content.replace(/~\/\.qwen(\/|$)/g, '~/.codex$1');
    content = content.replace(/\.\/\.qwen(\/|$)/g, './.codex$1');
