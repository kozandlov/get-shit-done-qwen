...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" from-gsd2
   ```
...
```bash
# Global
ln -s ~/.qwen/get-shit-done/skills/gsd-from-gsd2 ~/.qwen/skills/gsd-from-gsd2
