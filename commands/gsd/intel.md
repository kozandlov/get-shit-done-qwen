...
Check if intel is enabled by reading `.planning/config.json` directly using the read_file tool.
...
Intel system is disabled. To activate:
  ...
  node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs config-set intel.enabled true
...
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel query <term>
```
...
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel status
```
...
```bash
node $HOME/.qwen/get-shit-done/bin/gsd-tools.cjs intel diff
```
...
2. write_file JSON intel files to .planning/intel/ (stack.json, api-map.json, dependency-graph.json, file-roles.json, arch-decisions.json)
...
```bash
# Global
ln -s ~/.qwen/get-shit-done/skills/gsd-intel ~/.qwen/skills/gsd-intel
