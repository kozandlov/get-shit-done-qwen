...
- `tools` — Allowed tool access (read_file, write_file, edit, run_shell_command, grep_search, glob, web_search, etc.)
...
| `gsd-read-injection-scanner.js` | `PostToolUse` | Scans read_file tool output for injected instructions in untrusted content |
...
| `gsd-read-guard.js` | `PreToolUse` | Advisory guard preventing edit/write_file on files not yet read in the session |
...
Runtime Engine (Claude Code / Gemini CLI)
    ...
    └── SessionStart event ──► gsd-check-update.js
        Reads: VERSION file
        Writes: ~/.qwen/cache/gsd-update-check.json (spawns background process)
```
...
- Triggers on write_file/edit to `.planning/` files
...
- Triggers on write_file/edit to non-`.planning/` files
