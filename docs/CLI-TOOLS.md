...
```bash
# Generate skill manifest (writes to .qwen/skill-manifest.json)
...
# write_file a named snapshot of the current graph
...
gsd-sdk query config-set review.models.qwen   ""   # clear — fall back to session model
```
