...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `review.models.qwen` | string | (session model) | Command for Claude-flavored review. Defaults to the session model when unset |
...
| `workflow.text_mode` | boolean | `false` | Replaces ask_user_question TUI menus with plain-text numbered lists. Required for Claude Code remote sessions (`/rc` mode) where TUI menus don't render. Can also be set per-session with `--text` flag on discuss-phase. Added in v1.28 |
...
```xml
<agent_skills>
read_file these user-configured skills:
...
| `review.models.qwen` | string | (CLI default) | Model used when `--claude` reviewer is invoked |
