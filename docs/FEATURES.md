...
- [v1.32 Features](#v132-features)
  ...
  - [read_file-Before-edit Guard Hook](#73-read-before-edit-guard-hook)
...
**Requirements:**
- REQ-NOTE-01: System MUST save timestamped note files with a single write_file call
...
- REQ-NOTE-05: System MUST NOT use task, ask_user_question, or run_shell_command — runs inline only
...
The banner is silent when up-to-date and rate-limits "check failed" diagnostics to once per 24 hours. Removed cleanly by `npx gsd-qwen --uninstall` or by deleting the SessionStart entry that references `gsd-update-banner.js`.
...
PreToolUse hook that scans write_file/edit calls targeting `.planning/` for injection patterns. Advisory-only — logs detection for awareness without blocking legitimate operations.
...
3. **Output** — write_file `MILESTONE-SUMMARY.md` suitable for team onboarding and stakeholder review
...
2. **Migrate** — write_file `skills/gsd-*/SKILL.md` files for each GSD command
...
### 73. read_file-Before-edit Guard Hook
...
**Requirements:**
- REQ-RBE-01: Hook MUST detect edit/write_file tool calls that target files not previously read in the session
...
- REQ-SPIKE-06: `/gsd-spike-wrap-up` MUST package findings into `.qwen/skills/spike-findings-[project]/`
...
| `.qwen/skills/spike-findings-[project]/` | Packaged findings (via `/gsd-spike-wrap-up`) |
...
- REQ-SKETCH-06: `--quick` flag skips mood intake; `--text` flag replaces `ask_user_question` with numbered lists for non-Claude runtimes
- REQ-SKETCH-07: The winning variant MUST be marked in the README frontmatter and with a ★ in the HTML tab
- REQ-SKETCH-08: `/gsd-sketch-wrap-up` MUST package winning decisions into `.qwen/skills/sketch-findings-[project]/`
...
| `.qwen/skills/sketch-findings-[project]/` | Packaged decisions (via `/gsd-sketch-wrap-up`) |
