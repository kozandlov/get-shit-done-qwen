...
Once you have signal, run `/gsd-spike-wrap-up` to package the findings into `.qwen/skills/spike-findings-[project]/` — future sessions will load them automatically via project-skills discovery.
...
After picking a winner, run `/gsd-sketch-wrap-up` to capture the visual decisions into `.qwen/skills/sketch-findings-[project]/`.
...
- `gsd-prompt-guard.js` — Scans write_file/edit calls to `.planning/` for injection patterns (always active, advisory-only)
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" state validate          # Detect drift between STATE.md and filesystem
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" state sync --verify     # Preview what sync would change
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" state sync              # Reconstruct STATE.md from disk
```

These commands are new in v1.32 and replace manual STATE.md editing.

### read_file-Before-edit Infinite Retry Loop
...
```markdown
## edit Safety Rule
Always read a file before editing it. Never call edit or write_file on a file you haven't read in this session.
```
...
To enable this: run `/gsd-profile-user` to generate your profile. The profile is stored at `~/.qwen/get-shit-done/USER-PROFILE.md` and is read automatically on every `/gsd-discuss-phase` invocation. No other configuration is required.
...
This is a **harness setting**, not a GSD setting. The toggle lives in `.qwen/settings.json`:
...
If `npx gsd-qwen` fails due to npm outages or network restrictions, see [docs/manual-update.md](manual-update.md) for a step-by-step manual update procedure that works without npm access.
...
**Opt-out / removal:** delete the SessionStart hook entry that references `gsd-update-banner.js` from your runtime's `settings.json` (Claude Code: `~/.qwen/settings.json`; Gemini: `~/.gemini/settings.json`). `npx gsd-qwen --uninstall` removes both the script and the registration in one pass.
...
### Executor Subagent Gets "Permission denied" on run_shell_command Commands

GSD's `gsd-executor` subagents need write-capable run_shell_command access to a project's standard tooling — `git commit`, `bin/rails`, `bundle exec`, `npm run`, `uv run`, and similar commands. Claude Code's default `~/.qwen/settings.json` only allows a narrow set of read-only git commands, so a fresh install will hit "Permission to use run_shell_command has been denied" the first time an executor tries to make a commit or run a build tool.

**Fix: add the required patterns to `~/.qwen/settings.json`.**
...
"run_shell_command(git stash:*)",
"run_shell_command(git rm:*)",
"run_shell_command(git mv:*)",
"run_shell_command(git fetch:*)",
"run_shell_command(git cherry-pick:*)",
...
```json
"run_shell_command(bin/rails:*)",
"run_shell_command(bin/brakeman:*)",
"run_shell_command(bin/bundler-audit:*)",
"run_shell_command(bin/importmap:*)",
"run_shell_command(bundle:*)",
"run_shell_command(rubocop:*)",
"run_shell_command(erb_lint:*)"
```
...
**Example `~/.qwen/settings.json` snippet (Rails project):**
...
**Per-project permissions (scoped to one repo):** If you prefer to allow these patterns for a single project rather than globally, add the same `permissions.allow` block to `.qwen/settings.local.json` in your project root instead of `~/.qwen/settings.json`. Claude Code checks project-local settings first.

**Interactive guidance:** When an executor is blocked mid-phase, it will identify the exact pattern needed (e.g. `"run_shell_command(bin/rails:*)"`) so you can add it and re-run `/gsd-execute-phase`.
