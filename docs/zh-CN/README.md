...
如果你不想使用那个标志，在项目的 `.qwen/settings.json` 中添加：
...
OpenCode、Gemini CLI、Kilo 和 Codex 现在通过 `npx gsd-qwen` 原生支持。
