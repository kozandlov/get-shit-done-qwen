...
function cmdInitNewProject(cwd, raw) {
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.planning', '.qwen', '.codex', '__pycache__', 'target', 'dist', 'build']);
...
function cmdInitManager(cwd, raw) {
  ...
  // read_file manager flags from config (passthrough flags for each step)
...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  const os = require('os');
  const globalSkillsBase = path.join(os.homedir(), '.qwen', 'skills');
...
function buildSkillManifest(cwd, skillsDir = null) {
  ...
  const canonicalRoots = skillsDir ? [{
    ...
    {
      root: '.qwen/skills',
      path: path.join(cwd, '.qwen', 'skills'),
      scope: 'project',
      kind: 'skills',
    },
    {
    ...
    {
      root: '~/.qwen/skills',
      path: path.join(os.homedir(), '.qwen', 'skills'),
      scope: 'global',
      kind: 'skills',
    },
    {
    ...
    },
    {
      ...
      path: path.join(os.homedir(), '.qwen', 'get-shit-done', 'skills'),
      scope: 'import-only',
      kind: 'skills',
      deprecated: true,
    },
    {
      ...
      path: path.join(os.homedir(), '.qwen', 'commands', 'gsd'),
