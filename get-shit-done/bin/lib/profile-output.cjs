...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.qwen', 'skills', 'gsd-dev-preferences', 'SKILL.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  if (options.global) {
    ...
    // read_file claude_md_path from config, default to ./CLAUDE.md
    let configClaudeMdPath = './CLAUDE.md';
    try {
      const config = loadConfig(cwd);
      if (config.qwen_md_path) configClaudeMdPath = config.qwen_md_path;
    } catch { /* use default */ }
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    const config = loadConfig(cwd);
    if (config.qwen_md_path) configClaudeMdPath = config.qwen_md_path;
    if (config.qwen_md_assembly) assemblyConfig = config.qwen_md_assembly;
  } catch { /* use default */ }
