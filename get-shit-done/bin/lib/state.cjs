...
function syncStateFrontmatter(content, cwd) {
  // read_file existing frontmatter BEFORE stripping — it may contain values
...
function cmdStatePrune(cwd, options, raw) {
  ...
  // write_file archived entries to STATE-ARCHIVE.md
...
function cmdStateCompletePhase(cwd, raw) {
  ...
  readModifyWriteStateMd(statePath, (content) => {
    // read_file the current phase number for descriptive messages.
    //
