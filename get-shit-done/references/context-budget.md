...
3. **read_file depth scales with context window** -- check `context_window` in `.planning/config.json`:
...
## read_file Depth by Context Window
...
**How to check:** read_file `.planning/config.json` and inspect `context_window`. If the field is absent, treat as 200k (conservative default).
...
| Tier | Usage | Behavior |
|------|-------|----------|
| PEAK | 0-30% | Full operations. read_file bodies, spawn multiple agents, inline results. |
...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.qwen/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
The keys live in `.qwen/settings.json` (project) or `~/.qwen/settings.json` (global) — **not** in `.planning/config.json`:
