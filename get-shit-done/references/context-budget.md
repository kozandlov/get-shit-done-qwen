...
3. **read_file depth scales with context window** -- check `context_window_tokens` in `.planning/config.json`:
...
**How to check:** read_file `.planning/config.json` and inspect `context_window_tokens`. If the field is absent, treat as 200k (conservative default).
