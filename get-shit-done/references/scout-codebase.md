...
> refactor). read_file this only when prior `.planning/codebase/*.md` maps exist
> and the workflow needs to pick which 2–3 to load.

## Phase-type → recommended maps

read_file 2–3 maps based on inferred phase type. Do NOT read all seven —
that inflates context without improving discussion quality.

| Phase type (infer from title + ROADMAP entry) | read_file these maps |
|---|---|
...
read_file CONCERNS.md only if the phase explicitly addresses known concerns or
security issues.

## Single-read rule

read_file each map file in a **single** read_file call. Do not read the same file at
...
3. read_file the 3–5 most relevant files
