...
```bash
SELECTOR_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-framework-selector --raw)
RESEARCHER_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ai-researcher --raw)
DOMAIN_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-domain-researcher --raw)
PLANNER_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-eval-planner --raw)
```

Check config:
```bash
AI_PHASE_ENABLED=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
```markdown
read_file ~/.qwen/agents/gsd-framework-selector.md for instructions.
...
Copy template:
```bash
cp "$HOME/.qwen/get-shit-done/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
```markdown
read_file ~/.qwen/agents/gsd-ai-researcher.md for instructions.

<objective>
Research {primary_framework} for Phase {phase_number}: {phase_name}
write_file Sections 3 and 4 of AI-SPEC.md
</objective>
...
```markdown
read_file ~/.qwen/agents/gsd-domain-researcher.md for instructions.

<objective>
Research the business domain and expert evaluation criteria for Phase {phase_number}: {phase_name}
write_file Section 1b (Domain Context) of AI-SPEC.md
</objective>
...
```markdown
read_file ~/.qwen/agents/gsd-eval-planner.md for instructions.

<objective>
Design evaluation strategy for Phase {phase_number}: {phase_name}
write_file Sections 5, 6, and 7 of AI-SPEC.md
...
read_file the completed AI-SPEC.md. Check that:
