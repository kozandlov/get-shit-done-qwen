...
AGENT_SKILLS_CHECKER=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" phases list
```
...
## 2. read_file All Phase Verifications

For each phase directory, read the VERIFICATION.md:

```bash
# For each phase, use find-phase to resolve the directory (handles archived phases)
PHASE_INFO=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
