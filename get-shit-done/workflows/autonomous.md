...
```bash
SKIP_DISCUSS=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
write_file a minimal CONTEXT.md so downstream plan-phase has valid input. Get phase details:

```bash
DETAIL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```

Extract `goal` and `requirements` from JSON. write_file `${phase_dir}/${padded_phase}-CONTEXT.md` with:
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
