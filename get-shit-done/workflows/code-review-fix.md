...
read_file REVIEW.md findings, apply fixes, commit each atomically, write REVIEW-FIX.md. Do NOT commit REVIEW-FIX.md (orchestrator handles that).
")
```
...
Re-review the phase at ${REVIEW_DEPTH} depth. write_file findings to ${REVIEW_PATH}.
...
read_file REVIEW.md findings, apply fixes, commit each atomically, write REVIEW-FIX.md (overwrite previous). Do NOT commit REVIEW-FIX.md.
")
...
<platform_notes>
**Windows:** This workflow uses bash features (arrays, variable expansion, while loops). On Windows, it requires Git run_shell_command or WSL. Native PowerShell is not supported. The CI matrix (Ubuntu/macOS/Windows) runs under Git run_shell_command on Windows runners, which provides bash compatibility.
</platform_notes>
