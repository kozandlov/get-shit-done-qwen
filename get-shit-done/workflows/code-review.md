...
2. Config value: `node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
Review the listed source files at ${REVIEW_DEPTH} depth. write_file findings to ${REVIEW_PATH}.
...
read_file the REVIEW.md YAML frontmatter to extract finding counts.
...
Git run_shell_command or WSL. Native PowerShell is not supported. The CI matrix (Ubuntu/macOS/Windows)
runs under Git run_shell_command on Windows runners, which provides bash compatibility.
