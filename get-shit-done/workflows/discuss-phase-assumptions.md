...
- read_file the codebase FIRST, form opinions SECOND, ask ONLY about what's genuinely unclear
...
<answer_validation>
**IMPORTANT: Answer validation** — After every ask_user_question call, check if the response
...
When text mode is active, do not use ask_user_question at all. Present every question as a
...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-assumptions-analyzer 2>/dev/null)
```
...
**If matches found:** Present matched todos, use ask_user_question (multiSelect) to fold relevant ones into scope.
...
read_file the project-level methodology file if it exists. This must happen before assumption analysis
...
**If codebase maps exist:** read_file relevant ones (CONVENTIONS.md, STRUCTURE.md, STACK.md). Extract reusable components, patterns, integration points. Skip to Step 3.
...
read_file the 3-5 most relevant files.
...
Your job:
1. read_file ROADMAP.md phase {PHASE} description
2. read_file any prior CONTEXT.md files from earlier phases
3. glob/grep_search for files related to: {phase_relevant_terms}
4. read_file 5-15 most relevant source files
...
Use web_search for ecosystem/best-practice questions.
""")
```
...
- options:
  - "Yes, proceed" — write_file CONTEXT.md with these assumptions as decisions
...
Use ask_user_question (multiSelect):
- header: "Corrections"
...
Create phase directory if needed. write_file CONTEXT.md using the standard 6-section format.
...
write_file audit trail of assumptions and corrections.
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
2. Sync chain flag:
   ...
   ```
3. read_file chain flag and user preference:
   ```bash
