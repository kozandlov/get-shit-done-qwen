...
> `$HOME/.qwen/get-shit-done/USER-PROFILE.md` exists). Skip the read_file
...
If `ADVISOR_MODE` is false, do **not** read_file this file — proceed with the
standard `default.md` discussion flow.

## Calibration tier

Resolve `vendor_philosophy` calibration tier:
1. **Priority 1:** read_file `config.json` > `preferences.vendor_philosophy`
   (project-level override)
2. **Priority 2:** read_file USER-PROFILE.md `Vendor Choices/Philosophy` rating
   (global)
...
read_file USER-PROFILE.md and check for product-owner signals:
...
2. **Use ask_user_question** (or text-mode equivalent if `--text` overlay):
   - header: `{area_name}`
   - question: `Which approach for {area_name}?`
   - options: extract from the table's Option column (ask_user_question adds
     "Other" automatically)
...
5. **After recording pick, decide whether follow-up questions are needed:**
   - If the pick has ambiguity that would affect downstream planning →
     ask 1-2 targeted follow-up questions using ask_user_question
