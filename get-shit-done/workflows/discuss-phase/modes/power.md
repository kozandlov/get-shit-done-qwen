...
> **Lazy-loaded.** read_file this file from `workflows/discuss-phase.md` when
...
```
read_file @~/.qwen/get-shit-done/workflows/discuss-phase-power.md
```
...
- `--power --chain`: after the power-mode finalize step writes
  CONTEXT.md, the chain auto-advance still applies (read_file `chain.md`).
