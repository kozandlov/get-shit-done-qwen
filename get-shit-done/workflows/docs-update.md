...
```bash
INIT=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-doc-writer 2>/dev/null)
```
...
2. Otherwise, use ask_user_question to confirm:
...
1. **Scan the codebase for undocumented areas:**
   - Use glob/grep_search to discover significant source directories (e.g., `src/components/`, `src/pages/`, `src/services/`, `src/api/`, `lib/`, `routes/`)
...
Then confirm with ask_user_question:
...
3. If found at any path: mode = `"update"` — use the read_file tool to load the current file content (will be passed as `existing_content` in the doc_assignment block). Use the found path as the output path (do not move existing docs).
...
This table MUST be shown to the user — it is the primary confirmation of where files will be written and whether existing files will be updated. It appears as part of the queue presentation BEFORE the ask_user_question confirmation.
...
write_file the manifest using the write_file tool:
...
Present the following choice using `ask_user_question` if available, or inline prompt otherwise:
...
**Fallback when ask_user_question is unavailable:** Default all hand-written docs to `preserve` (safest default). Display message:

```
ask_user_question unavailable — hand-written docs preserved by default.
...
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — use `canonical_queue` items with `wave: 1` for this step.
...
write_file the doc file directly. Return confirmation only — do not return doc content."
)
```
...
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — update `status` to `"completed"` or `"failed"` for each Wave 1 item after collection. write_file the updated manifest back to disk.
...
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — use `canonical_queue` items with `wave: 2` for this step.
...
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — update `status` to `"completed"` or `"failed"` for each Wave 2 item after collection. write_file the updated manifest back to disk.
...
write_file {package_dir}/README.md directly. Return confirmation only — do not return doc content."
)
```
...
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — use `canonical_queue` items for generation order. Update `status` after each doc is generated. write_file the updated manifest back to disk after all docs are complete.
...
**IMPORTANT:** Do NOT use `browser_subagent`, `Explore`, or any browser-based tool. Use only file system tools (read_file, run_shell_command, write_file, grep_search, glob, or equivalent tools available in your runtime).

read_file `agents/gsd-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (read_file, grep_search, glob, run_shell_command) following gsd-doc-writer create_mode / update_mode instructions
   - write_file the file to the resolved path (README.md)
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - write_file the file to the resolved path (docs/ARCHITECTURE.md, or ARCHITECTURE.md if found at root as fallback)
...
3. **CONFIGURATION** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following gsd-doc-writer instructions
   - write_file the file to the resolved path (docs/CONFIGURATION.md, or CONFIGURATION.md if found at root as fallback)
...
- write_file the file to `{package_dir}/README.md`
...
**CRITICAL: read_file the work manifest first.**

```
read_file .planning/tmp/docs-work-manifest.json
```
...
2. read_file the result JSON from `.planning/tmp/verify-{doc_filename}.json`.
...
write_file the updated manifest back to disk.
...
<step name="fix_loop">
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — identify ALL docs (canonical AND non-canonical) with `claims_failed > 0` from the verification results in `.planning/tmp/verify-*.json`. Both queues are eligible for fixes.
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   a. read_file the current file content from disk.
...
2. After all fix agents complete, re-verify ALL docs (not just the ones that were fixed):
   ...
   - read_file updated result JSONs from `.planning/tmp/verify-{doc_filename}.json`.
...
1. For each doc in `existing_docs`:
   a. Spawn `gsd-doc-verifier` (or invoke sequentially if task tool is unavailable) with:
      ...
      ```
   b. read_file the result JSON from `.planning/tmp/verify-{doc_filename}.json`.
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: generate project documentation" \
...
<step name="report">
**read_file the work manifest first:** `read_file .planning/tmp/docs-work-manifest.json` — use the manifest to compile the complete report covering all canonical docs, review_queue results, and gap_queue results. The manifest is the source of truth for what was processed.
