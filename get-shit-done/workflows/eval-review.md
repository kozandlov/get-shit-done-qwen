...
```bash
AUDITOR_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-eval-auditor --raw)
```
...
**If `EVAL_REVIEW_FILE` non-empty:** Use ask_user_question:
...
```markdown
read_file ~/.qwen/agents/gsd-eval-auditor.md for instructions.
...
read_file the written EVAL-REVIEW.md. Extract:
- `overall_score`
