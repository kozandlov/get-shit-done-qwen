...
@~/.qwen/get-shit-done/references/context-budget.md
...
AGENT_SKILLS=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
read_file worktree config:

```bash
USE_WORKTREES=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
read_file context window size for adaptive prompt enrichment:
   ...
   for WT in $WORKTREES; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       if ! git diff --quiet .planning/STATE.md .planning/ROADMAP.md 2>/dev/null || \
         ...
         COMMIT_DOCS=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get commit_docs 2>/dev/null || echo "true")
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
   elif [ "${TEST_EXIT}" -eq 124 ]; then
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review 2>/dev/null || echo "true")
```
...
```bash
SCHEMA_DRIFT=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" verify schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
If `TEXT_MODE` is true, present as a plain-text numbered list. Otherwise use ask_user_question.
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
<files_to_read>
read_file these files before verification:
...
**Check config gate:**
```bash
GL_ENABLED=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" learnings copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
