...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.qwen/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.qwen/get-shit-done/references/planner-antipatterns.md
...
4. **Evaluate the result:**
   ...
   **Success (exit 0 + valid summary):**
   - read_file `$CANDIDATE_SUMMARY` and validate it contains meaningful content
     (not empty, has at least a heading and description — a valid SUMMARY.md structure)
   - write_file it as the plan's SUMMARY.md file
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   task(
     ...
     prompt="
       ...
       REQUIRED ORDER: write_file SUMMARY.md → commit → only then any narration. No text between write_file and commit (truncation risk; #2070 rescue is not primary defense).
       </parallel_execution>
       ...
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.qwen/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   # `.qwen/worktrees/agent-` (the namespace Claude Code's `isolation="worktree"`
   ...
   # read_file line-by-line so worktree paths containing whitespace are preserved (#2774).
...
5.6. **Post-merge build & test gate:**
   ...
   read_file and execute `get-shit-done/workflows/execute-phase/steps/post-merge-gate.md`.
...
read_file auto-advance config (chain flag OR user preference — same boolean as `check.auto-mode`):
```bash
...
2. read_file consolidated auto-mode (`active` = chain flag OR user preference; chain flag already synced in init step):
   ```bash
   AUTO_MODE=$(gsd-sdk query check auto-mode --pick active 2>/dev/null || echo "false")
   ```
