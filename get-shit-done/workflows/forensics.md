...
read_file these files if they exist:
...
read_file `.planning/reports/SESSION_REPORT.md` if it exists — extract last session outcomes,
...
read_file the current phase PLAN.md to determine expected file paths. Compare against
...
write_file to `.planning/forensics/report-$(date +%Y%m%d-%H%M%S).md`:
...
> - read_file specific files referenced in the evidence?
...
read_file additional files only if specifically needed.
