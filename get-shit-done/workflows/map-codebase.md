...
```bash
INIT=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
write_file these documents to .planning/codebase/:
- STACK.md - Languages, runtime, frameworks, dependencies, configuration
- INTEGRATIONS.md - External APIs, databases, auth providers, webhooks

Explore thoroughly. write_file documents directly using templates. Return confirmation only.
${AGENT_SKILLS_MAPPER}"
)
```
...
write_file this document to .planning/codebase/:
- CONCERNS.md - Tech debt, bugs, security, performance, fragile areas

Explore thoroughly. write_file document directly using template. Return confirmation only.
${AGENT_SKILLS_MAPPER}"
)
```
...
**IMPORTANT:** Do NOT use `browser_subagent`, `Explore`, or any browser-based tool. Use only file system tools (read_file, run_shell_command, write_file, grep_search, glob, list_dir, view_file, grep_search, or equivalent tools available in your runtime).
...
- write_file `.planning/codebase/STACK.md` — Languages, runtime, frameworks, dependencies, configuration
- write_file `.planning/codebase/INTEGRATIONS.md` — External APIs, databases, auth providers, webhooks

**Pass 2: Architecture Focus**
- Explore directory structure, entry points, module boundaries, data flow
- write_file `.planning/codebase/ARCHITECTURE.md` — Pattern, layers, data flow, abstractions, entry points
- write_file `.planning/codebase/STRUCTURE.md` — Directory layout, key locations, naming conventions

**Pass 3: Quality Focus**
- Explore code style, error handling patterns, test files, CI config
- write_file `.planning/codebase/CONVENTIONS.md` — Code style, naming, patterns, error handling
- write_file `.planning/codebase/TESTING.md` — Framework, structure, mocking, coverage

**Pass 4: Concerns Focus**
- Explore TODOs, known issues, fragile areas, security patterns
- write_file `.planning/codebase/CONCERNS.md` — Tech debt, bugs, security, performance, fragile areas
...
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
