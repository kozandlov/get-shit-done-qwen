...
read_file all files that exist. Missing files are fine — the summary adapts to what's available.
...
- read_file `{phase_dir}/{padded}-SUMMARY.md` if it exists — extract `one_liner`, `accomplishments`, `decisions`
- read_file `{phase_dir}/{padded}-VERIFICATION.md` if it exists — extract status, gaps, deferred items
- read_file `{phase_dir}/{padded}-CONTEXT.md` if it exists — extract key decisions from `<decisions>` section
- read_file `{phase_dir}/{padded}-RESEARCH.md` if it exists — note what was researched
...
read_file STATE.md and extract the `started_at` or earliest session date. Use it as the `--since` boundary:
```bash
...
write_file to `.planning/reports/MILESTONE_SUMMARY-v${VERSION}.md`:
...
write_file the summary, then commit:
```bash
