...
read_file project state to determine current position:

```bash
# Get state snapshot
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
read_file the file, resolve the issue, then delete it to continue.
