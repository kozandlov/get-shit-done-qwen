...
<step name="write">
**write_file handoff to the path determined in the detect step** (e.g. `.planning/phases/XX-name/.continue-here.md`, `.planning/spikes/SPIKE-NNN/.continue-here.md`, or `.planning/.continue-here.md`):
...
# BLOCKING CONSTRAINTS — read_file Before Anything Else
...
<step name="commit">
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
