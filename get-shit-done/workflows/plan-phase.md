...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.qwen/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
read_file ephemeral chain flag (same field as `check.auto-mode` → `auto_chain_active`):
```bash
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.qwen/get-shit-done/references/tdd.md to all eligible tasks:
...
task(
  ...
  **Chunked mode: outline-only.**
  ...
  write_file only: {PHASE_DIR}/{PADDED_PHASE}-PLAN-OUTLINE.md
...
Handle return:
- **`## OUTLINE COMPLETE`:** read_file `PLAN-OUTLINE.md`, extract plan list. Continue to 8.5.2.
...
3. Spawn the planner in **single-plan** mode — it must write exactly one PLAN.md file:
   ...
   task(
     ...
     **Chunked mode: single-plan.**
     write_file exactly ONE plan file: {PHASE_DIR}/{plan_id}-PLAN.md
...
**If "Prioritize":** Use ask_user_question (multiSelect) to let user pick which items are "now" vs "later". Create CONTEXT.md for each sub-phase with the selected items.
...
Use ask_user_question for each gap (or batch if multiple gaps).
...
ask_user_question. Selecting "Proceed anyway" continues to step 13b but
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
