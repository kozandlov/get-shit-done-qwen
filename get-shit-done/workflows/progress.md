...
read_file STATE.md `status` / `stopped_at` fields (from the STATE snapshot already loaded). Compare against the artifact count from the roadmap analysis. If STATE.md claims the current phase is pending/mid-flight but the artifact count shows it as complete (all PLAN.md files have matching SUMMARY.md files), flag inconsistency. Emit:
...
- ⚠ `Orphaned handoff files found` — list each file path, add: `→ Work was paused mid-flight. read_file the handoff before continuing.`
...
- Check 2 (orphaned handoff): `read_file the handoff file(s) and resume from where work was paused: /gsd-resume-work ${GSD_WS}`
