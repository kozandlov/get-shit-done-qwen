...
read_file from init: `phase_dir`, `phase_number`, `padded_phase`.
...
write_file to a temp file: `/tmp/gsd-review-prompt-{phase}.md`
</step>

<step name="invoke_reviewers">
read_file model preferences from planning config. Null/missing values fall back to CLI defaults.

```bash
GEMINI_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get review.models.gemini --raw 2>/dev/null || true)
CLAUDE_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get review.models.qwen --raw 2>/dev/null || true)
CODEX_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get review.models.codex --raw 2>/dev/null || true)
OPENCODE_MODEL=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" config-get review.models.opencode --raw 2>/dev/null || true)
```
...
Note: CodeRabbit reviews the current git diff/working tree — it does not accept a prompt or model flag. It may take up to 5 minutes. Use `timeout: 360000` on the run_shell_command tool call.
...
Commit:
```bash
node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
