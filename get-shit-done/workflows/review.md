...
CLAUDE_MODEL=$(gsd-sdk query config-get review.models.qwen 2>/dev/null | jq -r '.' 2>/dev/null || true)
...
read_file host and model from config. All three local backends share the same `/v1/chat/completions` endpoint — only host and model differ. Use `jq --rawfile` to safely encode the multi-line prompt as JSON without shell-escaping issues.
