...
```bash
INIT=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" init map-codebase 2>/dev/null || echo "{}")
...
```
task(
  prompt="Scan this codebase with focus: {focus}. write_file results to .planning/codebase/. Produce only: {document_list}",
