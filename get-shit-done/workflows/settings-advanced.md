...
`ask_user_question` call below with a plain-text numbered list and ask the user to type the
...
read_file `runtime` from the config (or treat as `"claude"` if absent). Look up the built-in
