...
- **Masking convention: `****<last-4>`** (e.g. `sk-abc123def456` → `****f456`).
  ...
  as `(unset)`.
- **Plaintext is never echoed by ask_user_question descriptions, confirmation
...
read_file the current config and compute a masked view for display. For each
...
`TEXT_MODE=true` and replace every `ask_user_question` call with a plain-text
...
descriptions or confirmation text. write_file the value via:
...
ask_user_question([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Claude", description: "review.models.qwen — defaults to session model when unset" },
...
Show the current value if any, offer Leave / Replace / Clear. write_file via:
