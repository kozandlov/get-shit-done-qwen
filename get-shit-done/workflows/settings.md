...
Use ask_user_question with current values pre-selected. Questions are grouped into six visual sections; the first question in each section carries the section-denoting `header` field (ask_user_question renders abbreviated section tags for grouping, max 12 chars).
...
ask_user_question([
  ...
  // the ask_user_question call and do not touch the existing workflow.code_review_depth value.
  {
...
write_file updated config to `$GSD_CONFIG_PATH` (the workstream-aware path resolved in `ensure_and_load_config`). Never hardcode `.planning/config.json` — workstream installs route to `.planning/workstreams/<slug>/config.json`.
</step>
