...
Use the project's language/framework by default. For greenfield projects with no existing stack, pick whatever gets to a runnable result fastest (Python, Node, run_shell_command, single HTML file).
...
**d.** write_file `README.md` with YAML frontmatter:
...
**e.** Auto-link related spikes: read existing spike READMEs and infer relationships from tags, names, and descriptions. write_file the `related` field silently.
