...
**Text mode:** If TEXT_MODE is enabled, replace ask_user_question calls with plain-text numbered lists.
</step>
...
**a. Conventions:** read_file `.planning/spikes/CONVENTIONS.md` if it exists.

**b. Findings skills:** glob for `./.qwen/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md` files.

**c. Manifest:** read_file `.planning/spikes/MANIFEST.md` for the index of all spikes.

**d. Related READMEs:** Based on the new idea, identify which prior spikes are related by matching tags, names, technologies, or domain overlap. read_file only those `.planning/spikes/*/README.md` files. Skip unrelated ones.
...
**b. Research the current state of the art.** Use context7 (resolve-library-id → query-docs) for libraries/frameworks. Use web search for APIs/services without a context7 entry. read_file actual documentation.
...
- **Surprising surface?** write_file a follow-up test that isolates and explores it.
...
**e.** write_file `README.md` with YAML frontmatter:
