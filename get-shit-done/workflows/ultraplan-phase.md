...
1. read_file the phase scope from ROADMAP.md — extract the goal, deliverables, and scope for
   the target phase.

2. read_file REQUIREMENTS.md if it exists (`requirements_path` is not null) — extract a
   concise summary (key requirements relevant to this phase, not the full document).

3. read_file RESEARCH.md if it exists (`research_path` is not null) — extract a concise
