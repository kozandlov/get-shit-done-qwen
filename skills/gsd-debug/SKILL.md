...
read_file TDD mode from config:
```bash
...
read_file file and print Current Focus block to console:
...
Create `.planning/debug/{slug}.md` with initial state using the write_file tool (never use heredoc):
