...
2. **read_file ROADMAP.md** and extract all 999.x phase entries:
   ```bash
...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" phase add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node "$HOME/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: review backlog — promoted N, removed M" --files .planning/ROADMAP.md
   ```
...
```bash
# Global
ln -s ~/.qwen/get-shit-done/skills/gsd-review-backlog ~/.qwen/skills/gsd-review-backlog
