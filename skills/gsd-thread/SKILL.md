...
- read_file frontmatter `status` field via:
  ```bash
...
- read_file frontmatter `updated` field for the last-updated date
- read_file frontmatter `title` field (or fall back to first `# Thread:` heading) for the title
...
2. read_file the file and display a summary:
   ```
   Thread: {SLUG}
   ─────────────────────────────────────
...
Resume the thread — load its context into the current session. read_file the file content and display it as plain text. Ask what the user wants to work on next.
...
3. Use the write_file tool to create `.planning/threads/{SLUG}.md` with this content:
